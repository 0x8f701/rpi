...
| Variable | Default | Purpose |
|----------|---------|---------|
| `PI_CODING_AGENT_DIR` | Default agent directory under the user's home | Agent config directory (`models.json`, `auth.json`, sessions, resources, llama cache) |
...
working directory is never trusted as a fallback.
...
| Variable | Purpose |
|----------|---------|
| `PI_BUN_EXECUTABLE` | Path to the Bun runtime executable |
| `PATH` | Used to discover `bun`/`bun.exe` when `PI_BUN_EXECUTABLE` is not set |
| `PI_EXTENSION_ID` | Extension id passed to Bun extensions (internal) |
