...
`pi` is a Rust port of the pi coding agent. It provides a self-contained CLI
...
See [`docs/install.md`](docs/install.md) for manual builds, verification, and
...
pi --print -m openai/gpt-5.5 "List the Rust files in this directory"

# JSON event stream (headless, one-shot)
pi --mode json -m anthropic/claude-sonnet-4-5 "List Rust files"
...
| Authentication via env vars, `auth.json`, `models.json`, `pi login`/`logout` | Implemented |
...
| Local/self-hosted models via `pi llama` + llama.cpp router | Implemented |
| JSON-RPC / stdio server (`--mode rpc`) | Implemented |
| Custom TUI themes and keybindings | Implemented |
| Local/git packages (`pi install/remove/list/update`) | Implemented |
| Process extension protocol via `pi-extension.json` manifests | Implemented |
| npm package backend | Not implemented |

## Subcommands

- `pi models [filter]` — list models
- `pi sessions` — list sessions
- `pi import-session SOURCE INPUT [--output PATH]` — convert external sessions
- `pi export SESSION_PATH [--output PATH] [--jsonl]` — export to HTML/JSONL
- `pi login [provider]` / `pi logout [provider]` — manage stored credentials
- `pi reload` — validate and print active resources
- `pi install SOURCE [--local]` / `pi remove SOURCE [--local]` / `pi list` — manage local/git packages
- `pi update [--self|--extensions] [PACKAGE]` — update pi or configured packages
- `pi llama configure|status|refresh|load|unload|search|details|download|installed` — manage local models
