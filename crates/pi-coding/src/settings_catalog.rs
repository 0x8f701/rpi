...
pub const SETTINGS_CATALOG: &[SettingDef] = &[
    ...
    setting!("live.realtimeModel", Live, STRING, "\"gpt-realtime-1.5\"", "Realtime model label sent over the Web realtime data channel (`session.update`); the create-call request itself omits `model` because Codex realtime rejects it. Used by the Web listener when live.mode is `realtime`; the runtime applies this default when the field is absent.", NONE, ALL, LIVE, false, false),
