...
| `rpc_client` | Wrapping the same application events as JSON-RPC 2.0 notifications. The CLI also ships a real `--mode rpc` stdio server; see [`src/user-guide/rpc-json.md`](../docs/src/user-guide/rpc-json.md). |
...
- [`quickjs_extension.mjs`](quickjs_extension.mjs) — an OMP-style default-export
  factory hosted by the in-process QuickJS runtime through a `runtime: "quickjs"`
  manifest.
